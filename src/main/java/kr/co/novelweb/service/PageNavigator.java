...
public class PageNavigator {
	...
	public PageNavigator(int countPerPage, int pagePerGroup, int currentPage, int totalRecordsCount) {
		...
		//현재 그룹
		currentGroup = (currentPage - 1) / pagePerGroup; //ex) 1페이지~5페이지 일때 그룹당페이지 수로 나누면  (5-1)/5 =0.~~~~~ 이기때문에 0그룹
		
		//현재 그룹의 첫 페이지
		startPageGroup = currentGroup * pagePerGroup + 1; //ex) 1그룹의 첫페이지 1*5+1 = 6 -> 6페이지
		//현재 그룹의 첫 페이지가 1보다 작으면 1로 처리
		startPageGroup = startPageGroup < 1 ? 1 : startPageGroup;
		//현재 그룹의 마지막 페이지
		endPageGroup = startPageGroup + pagePerGroup - 1; //앞서 구한 첫페이지값 + 그룹당 보여줄 페이지수 -1
